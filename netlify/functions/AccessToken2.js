...
// For Node.js environment, use zlib instead of pako
let pakoDeflate, pakoInflate, pakoInflateRaw;
if (typeof module !== 'undefined' && module.exports) {
    const zlib = require('zlib');
    pakoDeflate = (data) => zlib.deflateSync(Buffer.from(data));
    pakoInflate = (data) => zlib.inflateSync(Buffer.from(data));
    pakoInflateRaw = (data) => zlib.inflateRawSync(Buffer.from(data));
} else if (typeof pako !== 'undefined') {
    pakoDeflate = pako.deflate;
    pakoInflate = pako.inflate;
    pakoInflateRaw = pako.inflateRaw;
}
...
class AccessToken2 {
    ...
    async build() {
        ...
        let compressed = pakoDeflate(content)
    ...
    from_string(origin_token) {
        ...
        let buffer = pakoInflate(base64ToUint8Array(origin_content))
